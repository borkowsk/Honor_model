...
wb_dynmatrix<HonorAgent> HonorAgent::World;//Ca�y �wiat agent�w - wsp�lny

//Pomocnicze metody i akcesory agent�w
////////////////////////////////////////////////////////////////////////////////
...
*/
		...
		if(fabs(BULLI_POPUL)<1) // Oczywi�cie ma sens tylko wtedy  - jak 1 to sami agresywni - ALE NIGDYTAK NIE ROBIMY!
		{
		   ...
		   if(Agres!=1) //Jak nie jest
		   {
			 ...
			 {              //policyjny albo racjonalny - mog�oby by� CallPolice=1 i w ten spos�b znikaj� RACJONALNI (czwarta strategia)
			   if(!ONLY3STRAT)
...
//Zawsze kt�ry� musi zwyci�y�, co nie jest realistyczne - WA�NE UPROSZCZENIE
{
...
// Osobista zmiana reputacji, oraz ewentualnie rodzinna rekurencyjnie
{
	...
	//A teraz dla dzieci
	for(unsigned j=0;j<NeighSize();j++)//Czy jest mo�e ojcem? Trzeba szuka� potomk�w
	 if(Neighbourhood[j].Child==1) //Jest dzieckiem
	 {
		HonorAgent& Dziecko=World[Neighbourhood[j].Y][Neighbourhood[j].X];
		Dziecko.change_reputation_thru_family(Delta); //REKURENCJA W Dӣ RODZINY
	 }
}
...
//Jak MAFIAHONOR i go�� jest honorowy to zmienia si� tak samo ca�ej rodzinie!
{                                                   assert(this!=&Powod);//Sam siebie nie mo�e atakowa�!
	...
	if(&Powod!=NULL && MAFIAHONOR && !IsMyFamilyMember(Powod,Cappo) )//Je�eli dzia�a honor rodzinny to ...
	{
		if(Cappo==NULL)//Nie ma �yj�cego ojca - �lad si� urywa
				Cappo=this;
		Cappo->change_reputation_thru_family(Delta);//Ale mo�e mie� dzieci
	}
	else
	if(Delta>0) //Wzrost
	{
...
{                                  					assert(Power<=1);
	 ...
	 if(Power<0)
		Power=0;     //Czy to si� w og�le zdarza?
}
...
//Wyb�r partnera interakcji przez agenta, kt�ry dosta� losow� inicjatyw�
{
	...
	if(getNeigh(L,x,y) ) //Pobranie wsp�rz�dnych s�siada
	{
		...
		//BARDZO PROSTA REGU�A DECYZYJNA - gdy wygl�da, �e warto atakowa� i jest ch��    RULE
		if( (this->Agres==1
		||
		(this->Agres>0.0 && DRAND()<this->Agres))  //Czy w og�le jest agresywny i jak bardzo - jak w pe�ni to zawsze to przejdzie
		&&                                          //Agresywno�� jako  jako wyrachowanie
		( Ag.HonorFeiRep<=(RATIONALITY*this->Power+(1-RATIONALITY)*HonorFeiRep) //Gdy umownie S�ABSZY! Wi�c agresywni nigdy by tak ze sob� nie walczyli!     RULE
		||( DRAND()< AGRES_AGRESSION   )  )           //Agresywno�� spontaniczna (bez kalkulacji)
		)
		{                                                                       assert(this->Agres>0.0);
			if(  Ag.HonorFeiRep>this->HonorFeiRep )
			  AgressRandomAttack++;
				//cout<<"!"; //Zaatakowa� silniejszego  DEBUG
		...
		(Honor>0 && DRAND()<Honor*HONOR_AGRESSION) //Wi�c mo�e tylko honorowi maj� niezer� agresj� z powodu nieporozumie�?
		)
		{
			this->MemOfLastDecision=HOOK;//Ewentualna zaczepka losowa lub honorowa
			   HonorRandomAttack++;
			//cout<<"-"; //Zaatakowa� losowo/honorowo  DEBUG
		}
	}
